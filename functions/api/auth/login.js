...
export async function onRequestPost(context) {
  ...
  let passwordHash;
  try {
    passwordHash = await hashPassword(password, user.password_salt, user.password_iterations);
  } catch {
  ...
  await context.env.DB.prepare("DELETE FROM admin_sessions WHERE expires_at <= datetime('now')").run();

  return json({ ok: true, user: { username: user.username } }, 200, { "set-cookie": sessionCookie(token, SESSION_TTL_SECONDS) });
}
