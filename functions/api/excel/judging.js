...
export const RESULTS_SQL = `SELECT e.team_id,e.scores,e.nominations,
 ...
 COALESCE(lead.institution,'') leader_institution,
 ...
 LEFT JOIN hackathon_team_members lead ON lead.team_id=t.id AND lead.id=COALESCE(
 (SELECT id FROM hackathon_team_members WHERE team_id=t.id AND full_name=json_extract(e.team_snapshot,'$.leader_name')
 ORDER BY member_order,id LIMIT 1),
 t.attendance_lead_member_id,
 (SELECT id FROM hackathon_team_members WHERE team_id=t.id AND role='Captain' ORDER BY member_order,id LIMIT 1))
...
export async function onRequestGet(context) {
 ...
 const kind=params.get('kind'),sector=params.get('sector'),award=params.get('award'),category=params.get('category');
 if(category!==null && !['College','School'].includes(category))return json({ok:false,error:'Choose College or School.'},400);
 ...
 if(category){sql+=" AND COALESCE(json_extract(e.team_snapshot,'$.participant_category'),t.participant_category)=?";values.push(category);}
