...
export async function onRequestGet(context) {
  ...
  try {
    const results = await context.env.DB.batch([
      ...
      context.env.DB.prepare(`SELECT r.id, r.name, r.block, r.project_mode, r.sector, r.solution_type, vt.seats, vt.id AS table_id, vt.table_number, a.team_id, t.team_name, t.team_code
        FROM venue_rooms r JOIN venue_tables vt ON vt.room_id = r.id AND vt.table_number NOT BETWEEN 552 AND 568
...
export const MANUAL_SQL = `INSERT INTO venue_allocations (team_id, table_id, assigned_by)
 ...
 JOIN venue_tables vt ON vt.room_id = r.id AND vt.table_number NOT BETWEEN 552 AND 568 AND vt.seats >= (SELECT team_size FROM hackathon_teams WHERE id=q.team_id)
