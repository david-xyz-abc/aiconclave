    ...
    try {
      ...
      const statements = [
        ...
        db.prepare(`INSERT INTO hackathon_teams (team_code, team_name, team_name_key, captain_account_id, participant_category, team_size, sector_track, solution_type, information_confirmed, rules_accepted, updates_opt_in, submitted_at)
          SELECT ?, ?, ?, ?, ?, ?, ?, ?, 1, 1, ?, ?
          WHERE (${STUDENT_COUNT_SQL}) + ? <= ?`).bind(code, name, nameKey, participant.id, participantCategory, members.length, sectorTrack, solutionType, updatesOptIn ? 1 : 0, submittedAt, members.length, PUBLIC_STUDENT_LIMIT),
