...
export async function onRequestGet(context) {
  ...
  try {
    ...
    const result = await db.prepare(`SELECT id, name, email, phone, participant_type, organisation, department, panel_selection, industry_sector, industry_sector_other, organisation_type, organisation_type_other, updates_opt_in, created_at FROM panel_registrations WHERE participant_account_id = ? ORDER BY created_at DESC`).bind(session.id).all()
