...
export const COOKIE = '__Host-aiconclave_excel';
...
export async function authenticated({request,env}) {
 ...
 if (!raw || !env.EXCEL_SESSION_SECRET) return false;
 const parts=raw.split('.');
 if(parts.length!==3 || !/^\d+$/.test(parts[0]) || Number(parts[0])<=Date.now() || Number(parts[0])>Date.now()+43200000) return false;
 const expected=await signature(parts.slice(0,2).join('.'),env.EXCEL_SESSION_SECRET);
 const {constantTimeEqual}=await import('./auth.js');
