...
function AlreadyRegisteredPage({ eventName }) {
  ...
  return <main id="main">
    <section className={`page-header ${isHackathon ? 'hackathon-register-header' : 'panel-register-header'}`}><div className="container"><a className="back-link" href={PATHS.register}>← All registrations</a><p className="eyebrow">{isHackathon ? 'Day 2 · Hackathon' : 'Industry Panel Discussions'}</p><h1 className="section-heading">{eventName} Registration</h1><p className="panel-theme-line">Agriculture <span>•</span> {isHackathon ? 'Healthcare' : 'Education'} <span>•</span> {isHackathon ? 'Education' : 'Healthcare'}</p><p className="section-lede">{isHackathon ? 'Team registration for school and college students presenting technical or non-technical solutions.' : 'Industry panel discussions bringing together students, educators, researchers, professionals and delegates.'}</p></div></section>
