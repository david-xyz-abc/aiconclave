...
Judges sign in at `/judges/login`, see only their assigned teams and team leader names, and evaluate in three steps: scoring, sector nominations, and final review. The sector determines the six awards plus None of the above. Exactly one option must be explicitly selected before continuing or submitting. All sectors use Impact, Creativity, Validity, Relevance and Presentation, each scored with integer marks from 0–10 (50 total), with 1–10 in a 10-column or 5+5 grid and a separate zero option. Registration and seating details are readonly metadata.
...
Alpha: confirmed Not present submits zero scores, no award, and a not_present marker in the immutable team snapshot. The normal submission guard, ownership and audit history also apply. New snapshots store score_max=10; legacy snapshots default to 5. Historical submissions are not rewritten. Older drafts are proportionately scaled on next save. Results exports normalize to /10 and /50 for comparison.
