...
Judges sign in at `/judges/login`, see only their assigned teams and team leader names, and evaluate in three steps: scoring, award nomination, and final review. The sector determines the six awards plus None of the above. Exactly one option must be explicitly selected before continuing or submitting. All sectors use Impact, Creativity, Validity, Relevance and Presentation, each scored from 0–10 (50 total). Buttons show 1–10 with a separate No credit (0) option; selecting the same score again clears it. Registration and seating details are readonly metadata.

Save draft allows partial scores. Next saves all five scores before award nomination; the next step saves the nomination and opens final review. Submit details locks scores and nominations and returns the judge to the team list. A separate revision prevents competing tabs from overwriting a draft or submission. The server validates ownership, eligibility, score bounds, sector award IDs and completeness; the client cannot supply a total or override a submitted score.

Not present / Disqualify requires explicit confirmation, atomically submits zero for every criterion and None of the above, records the action in history, and locks the evaluation. Emergency reopening is required to change it. This records an evaluation outcome without changing check-in attendance.

Historical submissions retain their recorded five-point scale. When an older draft is edited, its marks are doubled once to preserve their proportions on the ten-point scale. Excel rankings normalize both scales to totals out of 50 without rewriting historical records.

The Evaluation Team can open or close new judge sign-ins using the login switch. Existing sessions remain valid and staff access is unaffected. Apply `0038_judge_login_control.sql` before deploying this feature. It initializes production to Open and preserves an existing setting when rerun. Availability and Backup badges use production's `slot_preference` and `roster_status` fields.
