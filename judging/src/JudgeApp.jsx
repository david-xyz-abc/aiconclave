  ...
  async function load() {
    ...
    try {
      ...
      if (teamId) {
        ...
        setScores(e?.scores || {});
        setStep(e?.status === "submitted" ? 2 : validScores(e?.scores, true) ? 1 : 0);
      }
      setDirty(false);
    } catch (e) {
  ...
  return (
    <>
      <header className="topbar">
        <a className="brand" href="https://aiconclave-dashboard.pages.dev/" aria-label="Home" title="Home" onClick={event => { if (dirty && !window.confirm("Return home and discard unsaved changes?")) event.preventDefault(); }}>
          ...
          <>
            ...
            {!locked && (
              <ol className="evaluation-steps" aria-label="Evaluation steps">
                {["Evaluation", "Nominations", "Review & submit"].map(
                  (label, index) => (
                    <li
                      key={label}
                ...
                <>
                  ...
                  <fieldset disabled={busy || data.routeNeedsReview}>
                    ...
                    <div className="form-actions">
                      ...
                      <button onClick={() => {
                        if (dirty && !window.confirm("Go back and discard unsaved nomination changes?")) return;
                        setNominations(saved?.nominations || []);
                        setDirty(false);
                        setStep(0);
                      }}>← Evaluation</button>
                      <button
