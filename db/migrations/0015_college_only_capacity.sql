-- Schools do not consume college capacity. Replace the global guards atomically
-- via the migration transaction; existing registrations are not modified.
...
BEGIN
  SELECT RAISE(ABORT, 'hackathon_capacity_exceeded') WHERE
    (SELECT COUNT(*) FROM hackathon_team_members m JOIN hackathon_teams t ON t.id = m.team_id WHERE t.submitted_at IS NOT NULL AND t.participant_category = 'College') >= 1300;
END;
...
BEGIN
  SELECT RAISE(ABORT, 'hackathon_capacity_exceeded') WHERE
    (SELECT COUNT(*) FROM hackathon_team_members m JOIN hackathon_teams t ON t.id = m.team_id WHERE t.submitted_at IS NOT NULL AND t.participant_category = 'College')
