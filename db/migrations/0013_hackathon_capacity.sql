...
BEGIN
  SELECT RAISE(ABORT, 'hackathon_capacity_exceeded') WHERE
    (SELECT COUNT(*) FROM hackathon_team_members m JOIN hackathon_teams t ON t.id = m.team_id WHERE t.submitted_at IS NOT NULL) >= 1200;
END;
...
BEGIN
  SELECT RAISE(ABORT, 'hackathon_capacity_exceeded') WHERE
    ...
    + (SELECT COUNT(*) FROM hackathon_team_members WHERE team_id = NEW.id) > 1200;
END;
