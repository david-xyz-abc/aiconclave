...
test('directory uses one compact data query with latest check-in totals; searching is local',async()=>{
 ...
 assert.equal(data.teams[0].present_count,0);assert.equal('email' in data.teams[0],false);
 ...
 f.sqlite.exec("INSERT INTO hackathon_attendance(team_id,member_id,attendance_date,present) VALUES(1,11,'2026-09-15',1),(1,11,'2026-09-16',0),(1,12,'2026-09-16',1)");
 const updated=await (await onRequestGet({env:{DB},request:new Request('https://test.example/api/attendance/teams',{headers:{cookie:'__Host-aiconclave_attendance_session=test'}})})).json();
 assert.equal(updated.teams[0].present_count,1);
